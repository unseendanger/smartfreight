...
export default function ContainerTabs({ activeId, onSelect, palletHeight }) {
  return (
    <div className="flex gap-1.5 p-1 rounded-lg bg-ink-800/70 border border-ink-700 w-fit">
      {CONTAINER_LIST.map((c) => {
        const displayHeight = c.id === 'ltl_pallet' && palletHeight ? palletHeight : c.height;
        return (
          <button
            key={c.id}
          ...
          >
            ...
            <span className="opacity-60 ml-1.5">
              {c.length}×{c.width}×{displayHeight}"
            </span>
          </button>
        );
      })}
